...
float n[] = { 0.0, -40.0, 0.0 }; // Normal bidang lantai
float e[] = { 0.0, -60.0, 0.0 }; // Posisi bidang lantai

// Fungsi untuk menggambar objek
void draw() {
    glutSolidCone(20, 50, 40, 50); // Menggambar sebuah kerucut
}
...
void glShadowProjection(float *l, float *e, float *n) {
    ...
    glMultMatrixf(mat); // Kalikan dengan matriks OpenGL
}

// Fungsi utama untuk merender objek dan bayangan
void render() {
    ...
    // Menampilkan sumber cahaya sebagai titik
    glBegin(GL_POINTS);
        glVertex3f(l[0], l[1], l[2]);
    glEnd();

    // Membuat lantai untuk bayangan
...
// Fungsi idle untuk rotasi objek
void idle() {
    rx += 0.1;
    ry += 0.1;
    render();
